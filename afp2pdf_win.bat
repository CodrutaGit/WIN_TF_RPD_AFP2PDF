...
set all_folders=e:\TestFiles\AFP\
...
echo %submition_tool%\itm_driver_win.exe -S %itm_ipaddress% -P 6986 -F"-itm_in_files %%i -itm_out_files e:\OutputTransformAFP2PDF\win\output\out_PDF\%%~pi\%%~ni.pdf" -s8040
%submition_tool%\itm_driver_win.exe -S %itm_ipaddress% -P 6986 -F"-itm_in_files %%i -itm_out_files  e:\OutputTransformAFP2PDF\win\output\out_PDF\%%~pi\%%~ni.pdf" -s8040 
 
 
 ) >> C:\Users\Administrator\Transforms\TestAFP2PDF\Log\log_win\all_afp2pdf_itm-driver.log 2>&1
echo ================================================
echo EXIT
echo EXIT